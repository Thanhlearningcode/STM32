...
DMA_Resigter::DMA_Resigter (const char* _name) 
{
    ...
    auto DMA_CCR = std::make_unique<Resigter>("DMA_CCR", 0x08);
    ...
    RegMap[0x08] = std::move(DMA_CCR);

    auto DMA_CMAR = std::make_unique<Resigter>("DMA_CMAR", 0x14);
    (*DMA_CMAR)("MA", 0, 31, e_Permit::ReadWrite, 0, 0);
    RegMap[0x14] = std::move(DMA_CMAR);

    auto DMA_CPAR = std::make_unique<Resigter>("DMA_CPAR", 0x10);
    (*DMA_CPAR)("PA", 0, 31, e_Permit::ReadWrite, 0, 0);
    RegMap[0x10] = std::move(DMA_CPAR);

    auto DMA_CNDTR = std::make_unique<Resigter>("DMA_CNDTR", 0x0C);
    (*DMA_CNDTR)("NDT", 0, 16, e_Permit::ReadWrite, 0, 0);
    RegMap[0x0C] = std::move(DMA_CNDTR);
}
